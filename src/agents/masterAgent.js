...
async function getRecentReports() {
  const { data } = await supabase
    ...
    .gte('created_at', new Date(Date.now() - POLL_INTERVAL_MS).toISOString())
    .order('created_at', { ascending: false })
    .limit(30);
  return data ?? [];
}

async function getLastMasterReport() {
  const { data } = await supabase
    .from('agent_reports')
...
export async function runMasterAgent() {
  ...
  // 긴급 상황이 없으면 마지막 마스터 보고가 30분 이내인 경우 스킵
  if (!hasUrgent) {
    const last = await getLastMasterReport();
    if (last && Date.now() - new Date(last.created_at) < 30 * 60 * 1000) return;
  }
